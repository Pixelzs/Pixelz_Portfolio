...
function cardSelectHandler(elements, modeClasses, addclass){
    return (e) => {
        e.stopPropagation();
        document.querySelectorAll('.card').forEach(c => c.classList.remove('selected'));
        e.currentTarget.classList.add('selected');
        removeAllClasses(elements, modeClasses);
        addClasses(elements, [addclass]);
...
let homeUrl = 'https://pixelzs.github.io/Pixelz_Portfolio/';
if(document.URL == homeUrl || document.URL.indexOf("index") > 0)
{
    ...
    document.addEventListener('DOMContentLoaded', () =>{
        ...
        document.querySelectorAll('#codeGrid .card').forEach(card => {
            card.addEventListener('click', cardSelectHandler(elements, modeClasses, modeClasses[1]));
        });


        document.querySelectorAll('#gameGrid .card').forEach(card => {
            card.addEventListener('click', cardSelectHandler(elements, modeClasses, modeClasses[0]));
        });
        ...
        viewprojBut.forEach(button => {
            button.addEventListener('click', (e) => {
                e.stopPropagation();
                let projId = button.getAttribute('projId');
        ...
        window.addEventListener('scroll', function() {
            ...
            const currentWindowScreenY = window.screenY;
            const navHeight = document.querySelector('header').offsetHeight;
            const elementShiftY = currentWindowScreenY + navHeight;
            if(currentScrollY <= 200) {
                ...
                if (currentScrollY > 220){
                    ...
                    homeEle.style.top = elementShiftY + "px";
